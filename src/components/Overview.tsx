...
export function Overview() {
  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      {/* Hero Section */}
      <motion.div 
        className="text-center py-6 sm:py-8 px-4 sm:px-6 rounded-2xl sm:rounded-3xl bg-gradient-to-br from-primary/90 via-primary to-accent/90 text-primary-foreground relative overflow-hidden"
        ...
        <div className="relative z-10">
          ...
          {!isMobile && (
            <div className="mt-4 flex justify-center items-center gap-6 text-sm">
              <div className="bg-primary-foreground/20 backdrop-blur-sm rounded-full px-4 py-2">
      ...
      <div className={`grid grid-cols-1 ${!isMobile ? 'lg:grid-cols-2' : ''} gap-4 sm:gap-6`}>
        ...
        {monthlyBudget > 0 && (
          ...
          >
            <Card className="bg-card/90 backdrop-blur-sm border-0 shadow-md card-hover">
                ...
                <div className="space-y-4">
                  ...
                  <div className="relative">
                    <Progress 
                      ...
                      className="h-3 sm:h-4 bg-muted progress-enhanced"
                    />
            ...
            <Card className="bg-card/90 backdrop-blur-sm border-0 shadow-md card-hover">
              ...
              <CardContent>
                <div className="space-y-4">
                  <div className="flex justify-between text-sm">
                    <span className="text-muted-foreground">Spent: <span className="font-medium text-foreground">{formatCurrency(totalSpent)}</span></span>
                    <span className="text-muted-foreground">Total: <span className="font-medium text-foreground">{formatCurrency(totalBudget)}</span></span>
                  </div>
                  <div className="relative">
                    <Progress 
      ...
      >
        ...
        {!isMobile && (
          <div className="flex items-center gap-2 text-sm text-muted-foreground bg-card/60 backdrop-blur-sm px-4 rounded-xl border border-border">
          ...
          <CardHeader className="pb-3">
            <CardTitle className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                <span>📋</span>
                <span className="text-base sm:text-lg text-foreground">Recent Expenses</span>
              </div>
            ...
            {recentExpenses.length > 0 ? (
              <div className="space-y-3">
                {recentExpenses.map((expense, index) => {
                  ...
                  return (
                    <motion.div 
                      key={expense.id} 
                      className="flex items-center justify-between p-3 sm:p-4 bg-gradient-to-r from-muted/50 to-muted/30 rounded-xl border border-border/50 hover:shadow-md transition-all duration-200 card-hover"
                      initial={{ opacity: 0, x: -20 }}
                      animate={{ opacity: 1, x: 0 }}
              ...
              <div className="text-center py-8 sm:py-12 text-muted-foreground">
                ...
                <Button
                  onClick={() => setShowAddExpense(true)}
                  variant="outline"
                  className="mt-4 border-dashed border-2 hover:bg-muted"
                >
